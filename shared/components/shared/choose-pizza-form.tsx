...
Форма выбора пиццы
*/
const ChoosePizzaForm: React.FC<Props> = ({ className, imageUrl, name, ingredients, items, onSubmit, loading }) => {
    ...
    return (
        <div className={cn(className, 'flex align-start flex-1 w-[1060px]')}>
            ...
            <div className="w-[490px] p-7 bg-[#fcfcfc] ">
                ...
                <Button loading={loading} className='h-[55px] px-10 text-base rounded-[18px] w-full mt-10' onClick={() => currentItemId&&onSubmit(currentItemId,Array.from(selectedIngredients))} >Добавить в корзину {total} ₽</Button>
            </div>
        </div>
    )
}
